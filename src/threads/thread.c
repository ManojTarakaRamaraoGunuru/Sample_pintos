...
thread_exit (void) 
{
  ...
  struct list cur_c=curr->children;
  if(curr!=NULL){
    for (e = list_begin(&thread_current()->children);e != list_end(&thread_current()->children); e = list_next(e)){
			if (list_entry(e, struct thread, child_elem)->process_exited)
      sema_up(&(list_entry(e, struct thread, child_elem))->sema_process_exit);
			else{
				list_entry(e, struct thread, child_elem)->parent = NULL;
				list_remove(&(list_entry(e, struct thread, child_elem))->child_elem);
			}
		}
  }
